...
export function TaskPicker({ projectTasks, onSelect, onCreate, onClose }: Props) {
  ...
  const filtered = q
    ? projectTasks.filter((t) => t.description.toLowerCase().includes(q))
    : projectTasks;

  // Show max 8 results
  const results = filtered.slice(0, 8);
  const exactMatch = projectTasks.some((t) => t.description.toLowerCase() === q);
        ...
        {results.map((task) => {
          ...
          return (
            ...
            >
              ...
              <span className="truncate text-gray-700">{task.description || "Untitled"}</span>
            </button>
          );
        })}
        {q && !exactMatch && (
